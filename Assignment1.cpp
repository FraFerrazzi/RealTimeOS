...
//This exercise show how to schedule four periodic threads with Rate Monotonic with semaphores using priority ceiling
...
int T1T2; // task1 writes and task2 reads
int T1T4; // task1 writes and task4 reads
int T2T3; // task2 writes and task3 reads
...
main()
{
	...
	// before doing anything else.
 	...
 	int i;
  	for (i =0; i < NTASKS; i++)
		...
		// In order to have a more attendible result i make the WCET for a given n cycles 
		// and get the maximum among the all WCET calculated.
			maxWCET[i] = 0;

			for (int j=0; j < 50; j++) {
				
      			compWCET[i]= 1000000000*(time_2.tv_sec - time_1.tv_sec)
			       +(time_2.tv_nsec-time_1.tv_nsec);
      			printf("WCET[%d]{%d} ; ", i, j);
				if ( compWCET[i] > maxWCET[i]){
					maxWCET[i] = compWCET[i];
				}
			}
			WCET[i] = maxWCET[i];
	...
	for (i=0; i < NTASKS; i++) {
		U = WCET[0]/periods[0]+WCET[1]/periods[1]+WCET[2]/periods[2];
	}
